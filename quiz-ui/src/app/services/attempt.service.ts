...
export class AttemptService {
  ...
  startAttempt(quizId: number): Observable<AttemptStartResponse> {
    return this.http.post<AttemptStartResponse>(`${this.apiUrl}/start/${quizId}`, {});
  }

  submitAttempt(attemptId: number, submission: AttemptSubmissionRequest): Observable<AttemptResult> {
    return this.http.post<AttemptResult>(`${this.apiUrl}/submit/${attemptId}`, submission);
  }
